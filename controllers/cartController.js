        ...
        element.products.forEach(async (el) => {
          if (el.productId.discount < el.productId.category.categoryDiscount) {
            ...
            const discountAmount =
              el.productId.price.regularprice *
              (el.productId.category.categoryDiscount / 100);
            const sellPrice = el.productId.price.regularprice - discountAmount;
            const sellingPrice = Math.floor(sellPrice);
            const dev = await Product.findByIdAndUpdate(
              { _id: el.productId._id },
              { $set: { "price.saleprice": sellingPrice } },
              { new: true }
            );
          }
        });
...
const addToCart = async (req, res) => {
  try {
    ...
    const userHaveCart = await myCart
    ...
    if (!userHaveCart) {
      ...
      const productIndex = userHaveCart.products.findIndex(
        (product) => product.productId._id == productId && product.size == sizee
      );

      if (productIndex !== -1) {
        const currqty = parseInt(userHaveCart.products[productIndex].quantity);
        const productqty = parseInt(
          userHaveCart.products[productIndex].productId.size[sizee].quantity
        );
        ...
        if (currqty + newqty <= productqty) {
          ...
          res
            .status(400)
            .json({ success: false, error: "Can't add more to cart" });
          return;
        }
      } else {
        ...
        if (parseInt(quantity) <= maxqty) {
          ...
          res
            ...
            .json({
              success: false,
              error: "quantity exceeds the limit of stock",
            });
          return;
        }
      }
...
const incQuantityUpdation = async (req, res) => {
  try {
    ...
    const cartData = await myCart
      .findOne({ userId: userId })
      .populate("products.productId");

    if (cartData) {
      const matchProduct = cartData.products.findIndex(
        (product) => product.productId._id == productId && product.size == sizey
      );
      if (matchProduct !== -1) {
        const matchprod = cartData.products.find(
          (prod) => prod.productId._id == productId && prod.size == sizey
        );
        if (matchprod) {
          ...
          if (currcartqty < maxqty) {
            ...
            res
              .status(400)
              .json({ success: false, updateError1: "reached the limit" });
            return;
          }
        } else {
      ...
      if (matchProduct !== -1) {
        ...
        if (matchprod) {
          ...
          if (currcartqty > minqty) {
            ...
            res
              .status(400)
              .json({ success: false, updateError2: "reached the limit" });
            return;
          }
        } else {
          console.log("cant find match prod");
...
const checkOut = async (req, res) => {
  try {
    ...
    if (cartOfUser) {
      ...
      res.render("cart", {
        cantMoveToCheckout:
          "Your cart has No proudcts,So cant proceed to checkout",
        cartEmpty: "Your cart is empty",
      });
    }
  } catch (error) {
