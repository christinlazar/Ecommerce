            ...
            if(totalprice<=wallet.balance){
             ...
             console.log("getting here in top of myOrder")
               ...
               let productDetArray = []
                ...
                const updatedWallet = await Wallet.findOneAndUpdate({userId:userId},{$inc:{balance:-totalprice}}).populate('userId')
                    res.status(200).json({success:true})
            }else{
                res.status(400).json({ success: false, error: "Total amount exceeds wallet balance" })
            }
       ...
       if(paymentMethod=="razorpay"||paymentMethod =="wallet"){
        ...
        remOrderArray.forEach(async(el)=>{
          ...
          await Product.findByIdAndUpdate({_id:el.product_id},{$inc:{[`size.${el.size}.quantity`]:el.qty}})
           ...
           const updatedWallet = await Wallet.findOneAndUpdate(filter,update,options).populate('userId')
...
const returnOrder = async(req,res)=>{
    try {
        ...
        const myOrder = await Order.findById(order)
        const totalAmount = myOrder.totalamount 
        await Order.findByIdAndUpdate(order,{$set:{totalAmount:0,status:"returned"}})
        let returnOrder = []
        ...
        returnOrder.forEach(async(element)=>{
            await Product.findByIdAndUpdate({_id:element.productId},{$inc:{[`size.${element.size}.quantity`]:element.qty}})
            const filter = {userId:req.session.user}
           const update = {$inc:{balance:totalAmount}}
           const options = {upsert:true}
           const updatedWallet = await Wallet.findOneAndUpdate(filter,update,options).populate('userId')
...
const downloadInvoice = async(req,res)=>{
    try {
        ...
        const order = await Order.findById(orderId).populate('addressId').populate('userId').populate('products.product')
