    ...
    const croppedImages = await Promise.all(
      images.map(async (image) => {
        ...
        const outputFilePath = path.join(
          ...
          )}_cropped${path.extname(inputFilePath)}`
        );
...
const productList = async (req, res) => {
  try {
    const category = await Category.find({ is_active: true }).sort({
      createdAt: -1,
    });
...
const loadOffers = async (req, res) => {
  try {
    ...
    const offerProductsToexpire = await Product.find({
      discount: { $gt: 0 },
      enddate: { $lt: today },
    });
    if (offerProductsToexpire) {
      offerProductsToexpire.forEach(async (el) => {
        await Product.updateMany(
          { _id: el._id },
          { $set: { discount: 0, startdate: today, endDate: today } }
        );
      });
    }
  } catch (error) {
