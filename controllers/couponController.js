...
const addCoupon = async (req, res) => {
  try {
    ...
    const { name, purchase, code, discount, mindiscount, expirationDate } =
      req.body;
...
const holdCoupon = async (req, res) => {
  try {
    ...
    const findedCoupon = await Coupon.findOneAndUpdate(
      { _id: couponId },
      { $set: { is_active: false } }
    );
...
const applyCoupon = async (req, res) => {
  try {
    ...
    const discountPercentage =
      Math.floor(
        Math.random() * (maxdiscountPercentage - mindiscountPercentage + 1)
    ...
    const discountAmount = minpurchase * (discountPercentage / 100);
      ...
      if (couponWithThisCode.users.includes(userId)) {
        ...
        res.render("checkout", {
          expired: "coupon has been expired",
          cartOfUser,
          addressOfUser,
          coupon,
        });
        ...
        res.render("checkout", {
          minpurchase: `Minimum purchase Amount is ${minpurchase}`,
          cartOfUser,
          addressOfUser,
          coupon,
        })
   ...
   const  addressOfUser = await address.find({ userId: userId });
        res.render('checkout',{invalid:"This coupon is not valid",cartOfUser,coupon,addressOfUser})
    }
  } catch (error) {
    console.log(error)
  }
