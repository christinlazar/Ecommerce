...
const verifyEditBanner = async(req,res)=>{
    try {
        ...
        if(newImages.length>0){
            await Banner.updateOne({_id:bannerId},{$push:{image:{$each:newImages}}})
        }
            await Banner.findByIdAndUpdate(bannerId,{$set:{
                title:title,
                description:description,
                startDate:startingDate,
                endDate:endingDate
...
const blockBanner = async(req,res)=>{
    try {
        const bannerId = req.body.bannerId
        await Banner.findOneAndUpdate({_id:bannerId},{$set:{isActive:false}})
...
const unBlockBanner = async(req,res)=>{
    try {
        const bannerId = req.body.bannerId
        await Banner.findOneAndUpdate({_id:bannerId},{$set:{isActive:true}})
        res.status(200).json({succces:true})
    } catch (error) {
        console.log(error)
    }
